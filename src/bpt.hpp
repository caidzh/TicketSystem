...
void fsta(const string &s,char str[]){
...
public:
    ...
    bool operator ==(const node &x)const{
        return (index==x.index&&(strcmp(val,x.val)==0));
    }
};
...
public:
    ...
    MemoryRiver<full_node>file_node;
    MemoryRiver<Arr>file_Arr;
    MemoryRiver<T>file_info;
...
bool findone(node &x,T &o){
    ...
    while(!cur.is_leaf){
        ...
        bool flg=false;
        for(int i=0;i<cur.size;i++)
            if(x.index<A.a[i].index||(x.index==A.a[i].index&&strcmp(x.val,A.a[i].val)<0)){
                file_Arr.read(cur,cur.son[i]);
                flg=true;
                break;
            }
    ...
    do{
        file_node.read(A,cur.arr);
        if(A.a[0].index>x.index||(A.a[0].index==x.index&&strcmp(A.a[0].val,x.val)>0))
            break;
        for(int i=0;i<cur.size;i++)
            if(A.a[i].index==x.index&&strcmp(A.a[i].val,x.val)==0){
                flg=true;
                file_info.read(o,A.a[i].info_pos);
            }
        if(cur.next!=-1)
...
public:
    string s;
    int M1,M2;
    bpt_s(int M1,int M2):s(""),M1(M1),M2(M2){}
    class full_node{
    public:
        Node a[M+1];
    };
...
bool find(Node &x,train_s a[],int &len){
    if(root==-1)
        return false;
    Arr cur;
    full_node A;
...
bool findone(Node &x,train_s &o){
    ...
    while(!cur.is_leaf){
        ...
        bool flg=false;
        for(int i=0;i<cur.size;i++)
            if(x.index<A.a[i].index||(x.index==A.a[i].index&&x.val<A.a[i].val)){
                file_Arr.read(cur,cur.son[i]);
                flg=true;
                break;
            }
    ...
    do{
        file_node.read(A,cur.arr);
        if(A.a[0].index>x.index||(A.a[0].index==x.index&&x.val<A.a[0].val))
            break;
        for(int i=0;i<cur.size;i++)
            if(A.a[i].index==x.index&&A.a[i].val==x.val){
                o=A.a[i].val;
                return true;
            }
        if(cur.next!=-1)
